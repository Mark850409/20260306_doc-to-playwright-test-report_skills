...
description: Generates test cases from project specification docs (e.g. docs/*.md), runs E2E tests via Playwright MCP (user-playwright), and produces a DOCX test report (YYYYMMDD_XXX測試報告.docx) with embedded screenshots and results. Use when the user asks to generate tests from docs, run automated E2E tests with Playwright, or produce a test report from specification documents.
---

# 依專案文檔產生測試案例並以 Playwright 執行與產出報告
...
5. 測試前確認前端服務已啟動；導頁時使用本專案約定 URL（首頁 `http://localhost:8080/ebg/`、表單文件下載 `http://localhost:8080/ebg/form-document-download`），詳見 [reference.md](reference.md)
...
- 執行 Phase 4 前請先**讀取並遵循 `docx` skill**（例如 `C:\Users\zanehsu\.claude\skills\docx\SKILL.md` 或專案內對應的 docx skill），依其規範建立文件（例如 docx-js 的 Document/Packer、或 unpack → 編輯 XML → pack）。
...
1. **格式**：使用 `.docx`，**一律透過 docx 技能**（docx-js 或 unpack/edit/pack）產生，禁止手寫產生 DOCX 的腳本或程式碼。
2. **截圖**：將 Phase 3 存下的截圖檔（如 `e2e-tc01-*.png`）**嵌入**報告內文，不要只寫路徑；每個測試案例區塊須含至少一張對應截圖；圖片嵌入方式依 docx skill 的 ImageRun / 或 XML 圖片規範。
3. **檔名**：`YYYYMMDD_XXX測試報告.docx`，其中 `YYYYMMDD` 為**執行當天日期**（例如 20260306），`XXX` 為報告類型（如 E2E、表單文件下載 等），範例：`20260306_E2E測試報告.docx`
...
- 報告結構、DOCX 產出與 MCP 工具參數：[reference.md](reference.md)
- **Word 文件產出**：必須使用 **`docx` skill**（讀取並遵循其 SKILL.md），依其規範建立 .docx（嵌入圖片、表格、標題、頁碼等），不得自行撰寫產生文檔的程式碼
